...
const repeatLabel=t=>{if(!t.repeat||t.repeat==="none")return null;if(t.repeat==="weekly")return`🔄 매주 ${DAYS[new Date(t.due+"T00:00:00").getDay()]}`;if(t.repeat==="monthly")return`🔄 매월 ${parseInt(t.due.slice(8))}일`;if(t.repeat==="weekdays")return"🔄 평일";if(t.repeat==="custom")return`🔄 ${(t.repeatDays||[]).sort().map(d=>DAYS[d]).join("·")}`;return null;};
const isActiveToday=t=>{if(!t.due||t.due>todayStr)return false;const now=new Date();const dow=now.getDay();if(!t.repeat||t.repeat==="none")return t.due===todayStr;if(t.repeat==="weekly")return new Date(t.due+"T00:00:00").getDay()===dow;if(t.repeat==="monthly")return parseInt(t.due.slice(8))===now.getDate();if(t.repeat==="weekdays")return dow>=1&&dow<=5;if(t.repeat==="custom")return(t.repeatDays||[]).includes(dow);return false;};
const expandForMonth=(tasks,y,m)=>{const dim=new Date(y,m+1,0).getDate(),res=[];tasks.forEach(t=>{if(!t.due||!t.repeat||t.repeat==="none"){res.push(t);return;}const sd=t.due;if(t.repeat==="weekly"){const dow=new Date(t.due+"T00:00:00").getDay();for(let d=1;d<=dim;d++){const date=`${y}-${String(m+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;if(date<sd)continue;if(new Date(y,m,d).getDay()===dow)res.push({...t,id:t.id+"-w"+d,due:date,_ir:true});}}else if(t.repeat==="monthly"){const day=parseInt(t.due.slice(8));if(day<=dim){const date=`${y}-${String(m+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;if(date>=sd)res.push({...t,due:date,_ir:true});}}else if(t.repeat==="weekdays"){for(let d=1;d<=dim;d++){const date=`${y}-${String(m+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;if(date<sd)continue;const dow=new Date(y,m,d).getDay();if(dow>=1&&dow<=5)res.push({...t,id:t.id+"-wd"+d,due:date,_ir:true});}}else if(t.repeat==="custom"){const days=t.repeatDays||[];for(let d=1;d<=dim;d++){const date=`${y}-${String(m+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;if(date<sd)continue;if(days.includes(new Date(y,m,d).getDay()))res.push({...t,id:t.id+"-c"+d,due:date,_ir:true});}}});return res;};
...
function RepeatPicker({repeat,repeatDays,due,onChange}){
  ...
  return(<div><select value={repeat} onChange={e=>onChange("repeat",e.target.value)} style={{border:"1px solid #e5e7eb",borderRadius:10,padding:"9px 12px",fontSize:13,background:"#fff",width:"100%"}}>{opts.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}</select>{repeat==="weekly"&&due&&<div style={{marginTop:6,fontSize:12,color:"#7c3aed",background:"#f5f3ff",borderRadius:8,padding:"5px 10px"}}>📅 매주 <b>{dueDow}요일</b> ({due} 부터)</div>}{repeat==="monthly"&&due&&<div style={{marginTop:6,fontSize:12,color:"#7c3aed",background:"#f5f3ff",borderRadius:8,padding:"5px 10px"}}>📅 매월 <b>{parseInt(due.slice(8))}일</b> ({due} 부터)</div>}{repeat==="weekdays"&&due&&<div style={{marginTop:6,fontSize:12,color:"#7c3aed",background:"#f5f3ff",borderRadius:8,padding:"5px 10px"}}>📅 월~금 평일 ({due} 부터)</div>}{repeat==="custom"&&(<div style={{marginTop:8}}><div style={{fontSize:12,color:"#6b7280",marginBottom:5}}>반복할 요일 ({due} 부터)</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{DAYS.map((d,i)=>(<button key={i} onClick={()=>toggleDay(i)} style={{width:36,height:36,borderRadius:"50%",border:`2px solid ${(repeatDays||[]).includes(i)?"#7c3aed":"#e5e7eb"}`,background:(repeatDays||[]).includes(i)?"#7c3aed":"#fff",color:(repeatDays||[]).includes(i)?"#fff":"#374151",fontSize:13,fontWeight:600,cursor:"pointer"}}>{d}</button>))}</div>{(repeatDays||[]).length>0&&<div style={{marginTop:6,fontSize:12,color:"#7c3aed"}}>선택: {(repeatDays||[]).sort().map(d=>DAYS[d]).join(", ")}요일</div>}</div>)}</div>);
}
...
function MainApp({user,onLogout}){
  ...
  const selDayCE=useMemo(()=>filterCE(calCE.filter(e=>e.date===selectedDay)),[calCE,selectedDay,filterCE]);
  ...
  const selDayLabel=selectedDay?new Date(selectedDay+"T00:00:00").toLocaleDateString("ko-KR",{month:"long",day:"numeric",weekday:"short"}):"";

  return(
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"'Pretendard','Apple SD Gothic Neo',sans-serif",background:"#f8fafc"}}>
      ...
      <div style={{flex:1,minWidth:0,padding:"24px 28px",overflowY:"auto"}}>
        ...
        {/* 진행률 바 */}
        <div style={{background:"#fff",borderRadius:14,padding:"14px 18px",marginBottom:20,border:"1px solid #e5e7eb",display:"flex",alignItems:"center",gap:24}}>
          <div style={{flex:1}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:12,color:"#6b7280"}}>전체 작업 진행률</span><span style={{fontSize:12,fontWeight:700,color:"#2563eb"}}>{done}/{tasks.length} 완료 ({pct}%)</span></div>
          ...
          <div style={{display:"flex",gap:20,flexShrink:0}}>
            {Object.entries(S).map(([k,v])=>(<div key={k} style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:v.color}}>{tasks.filter(t=>t.status===k).length}</div><div style={{fontSize:11,color:"#9ca3af"}}>{v.label}</div></div>))}
            <div style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:"#7c3aed"}}>{contracts.length}</div><div style={{fontSize:11,color:"#9ca3af"}}>계약</div></div>
          </div>
        </div>
        ...
        {tab==="list"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <h2 style={{margin:0,fontSize:18,fontWeight:700,color:"#111827"}}>📋 작업 목록</h2>
              <button onClick={()=>{setEditTaskData(null);setForm(EF(user.isAdmin));setShowForm(v=>!v);}} style={{background:"#2563eb",color:"#fff",border:"none",borderRadius:10,padding:"8px 18px",fontSize:13,fontWeight:700,cursor:"pointer"}}>{showForm&&!editTaskData?"✕ 닫기":"+ 새 작업"}</button>
            </div>
        ...
        {tab==="calendar"&&(
          <div>
            <h2 style={{margin:"0 0 16px",fontSize:18,fontWeight:700,color:"#111827"}}>📅 캘린더</h2>
            <div style={{display:"grid",gridTemplateColumns:"1fr 340px",gap:20,alignItems:"start"}}>
              {/* 캘린더 */}
              <div style={{background:"#fff",borderRadius:14,padding:18,border:"1px solid #e5e7eb"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                  <button onClick={()=>{let m=calM-1,y=calY;if(m<0){m=11;y--;}setCalM(m);setCalY(y);}} style={{background:"none",border:"1px solid #e5e7eb",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontSize:17}}>‹</button>
                  <div style={{fontWeight:800,fontSize:17}}>{calY}년 {calM+1}월</div>
                  <button onClick={()=>{let m=calM+1,y=calY;if(m>11){m=0;y++;}setCalM(m);setCalY(y);}} style={{background:"none",border:"1px solid #e5e7eb",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontSize:17}}>›</button>
                </div>
                  ...
                  {cells.map((day,i)=>{
                    ...
                    return(
                      <div key={i} onClick={()=>setSelectedDay(isSel?null:ds)}
                        style={{minHeight:72,background:isSel?"#eff6ff":isToday?"#f0f9ff":"#fff",border:`1.5px solid ${isSel?"#2563eb":isToday?"#93c5fd":"#f0f0f0"}`,borderRadius:10,padding:"5px 4px",cursor:"pointer",transition:"all .15s"}}>
              ...
              {/* 오른쪽 일별 상세 패널 */}
              <div style={{background:"#fff",borderRadius:14,border:"1px solid #e5e7eb",overflow:"hidden",position:"sticky",top:24}}>
                <div style={{padding:"14px 16px",borderBottom:"1px solid #e5e7eb",background:selectedDay===todayStr?"#eff6ff":"#f8fafc",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:14,color:"#111827"}}>{selectedDay?selDayLabel:"날짜를 선택하세요"}</div>
                    {selectedDay===todayStr&&<div style={{fontSize:11,color:"#2563eb",fontWeight:600,marginTop:1}}>오늘</div>}
                  </div>
                  {selectedDay&&<span style={{fontSize:12,color:"#9ca3af"}}>{selDayTasks.length+selDayCE.length}개 일정</span>}
                </div>
                <div style={{padding:"14px 16px",maxHeight:"calc(100vh - 300px)",overflowY:"auto"}}>
                  {!selectedDay&&<div style={{textAlign:"center",padding:"40px 0",color:"#d1d5db",fontSize:13}}>👈 캘린더에서 날짜를 클릭하세요</div>}
                  {selectedDay&&selDayTasks.length===0&&selDayCE.length===0&&(
                    <div style={{textAlign:"center",padding:"40px 0",color:"#9ca3af",fontSize:13}}>이 날 일정이 없어요 😊</div>
                  )}
                  {selectedDay&&(
        ...
        {tab==="contracts"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <h2 style={{margin:0,fontSize:18,fontWeight:700,color:"#111827"}}>🤝 계약 관리</h2>
              {user.isAdmin&&<button onClick={()=>{setEditContract(null);setShowCF(v=>!v);}} style={{background:"#7c3aed",color:"#fff",border:"none",borderRadius:10,padding:"8px 18px",fontSize:13,fontWeight:700,cursor:"pointer"}}>{showCF&&!editContract?"✕ 닫기":"+ 계약 등록"}</button>}
            </div>
        ...
        {tab==="report"&&(
          <div>
            <h2 style={{margin:"0 0 16px",fontSize:18,fontWeight:700,color:"#111827"}}>📊 업무 보고</h2>
